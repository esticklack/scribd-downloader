...
# Salida en UTF-8 para que los acentos/ñ de los titulos no se rompan al
# capturarse desde PowerShell.
try:
...
def progress(percent, message=""):
    """
    Emite una linea de progreso legible por la barra de PowerShell.
    Formato: @@P@@ <0-100> <mensaje>
    """
...
def convert_scribd_link(url):
    ...
    Returns:
        ...
        can be extracted.
    """
    # Acepta cualquier subdominio (www., es., etc.) y document/doc/presentation,
    # con o sin barra final.
...
def sanitize_filename(name, max_length=120):
    """
    Convierte un titulo en un nombre de archivo valido para Windows.
    Quita caracteres prohibidos, colapsa espacios y recorta la longitud.
    """
    name = name.strip()
    # Caracteres no permitidos en nombres de archivo de Windows.
    name = re.sub(r'[<>:"/\\|?*\x00-\x1f]', "", name)
    # Espacios/guiones repetidos -> un espacio.
    name = re.sub(r"\s+", " ", name).strip()
    # Windows no permite terminar en punto o espacio.
    name = name.rstrip(". ")
    if len(name) > max_length:
        name = name[:max_length].rstrip(". ")
    return name or "scribd_document"


def get_document_title(driver, document_url):
    """
    Obtiene el titulo real del documento cargando su pagina en Scribd.
    Devuelve el titulo limpio, o None si no se puede determinar.
    """
    try:
        ...
        if not title:
            return None
        # Scribd anade sufijos como " | PDF | Categoria | ..." -> nos quedamos
        # con la primera parte (el titulo).
        title = title.split(" | ")[0].strip()
        return title or None
    except Exception as error:
        print(f"No se pudo obtener el titulo del documento: {error}")
        return None


def extract_page_image_urls(driver):
    """
    Extrae las URLs de las imagenes de pagina del embed (documentos de imagen
    y escaneados). Devuelve un dict {numero_de_pagina: url}.
    """
    urls = driver.execute_script(
        ...
        Array.from(document.images).forEach(im => {
            ...
            const m = u.match(/\/images\/(\d+)-[a-f0-9]+\.jpg/);
            if (m) set[parseInt(m[1])] = u;
        });
        return set;
        """
    )
...
def is_image_document(image_urls):
    """
    True si el documento es de imagenes: hay imagenes de pagina y cubren de
    forma contigua desde la pagina 1 (sin huecos). Asi distinguimos un libro
    de imagenes / escaneo de un documento de texto (que no tiene estas imagenes).
    """
    if not image_urls:
        return False
    paginas = set(image_urls.keys())
    return paginas == set(range(1, max(paginas) + 1))


def download_images_to_pdf(image_urls, pdf_path, referer):
    """
    Descarga las imagenes de pagina en orden y las une en un PDF SIN recomprimir
    (con img2pdf). Conserva la calidad original y produce archivos mucho mas
    ligeros que el print-to-PDF. Devuelve la ruta absoluta del PDF.
    """
    import img2pdf

    headers = {"User-Agent": USER_AGENT, "Referer": referer}
    paginas = sorted(image_urls.keys())
    total = len(paginas)
    session = requests.Session()
    imagenes = []
    for i, n in enumerate(paginas, 1):
        resp = session.get(image_urls[n], headers=headers, timeout=30)
        resp.raise_for_status()
        imagenes.append(resp.content)
        progress(25 + 70 * i / total, f"Descargando imagen {i}/{total}")
        if i % 10 == 0 or i == total:
            print(f"  Descargadas {i}/{total} imagenes")

    progress(97, "Armando PDF")
    with open(pdf_path, "wb") as out:
        out.write(img2pdf.convert(imagenes))
    ...
    while stable_rounds < 2:
        ...
        for index in range(scrolled_count, total_pages):
            ...
            if (index + 1) % 10 == 0:
                ...
                # Anima la barra durante el scroll dentro de la banda 20-25%.
                progress(20 + 5.0 * (index + 1) / total_pages,
                         f"Cargando paginas {index + 1}/{total_pages}")
...
def main():
    """Run the downloader. Accepts the URL as a CLI argument or asks for it."""
    # URL desde la línea de comandos (p. ej. `s <url>`) o, si no, por teclado.
    ...
    with tempfile.TemporaryDirectory(prefix="scribd-chrome-profile-") as runtime_profile_dir:
        ...
        try:
            progress(3, "Iniciando navegador")
            print("\nStarting Chrome browser...")
            options = build_chrome_options(runtime_profile_dir)
            driver = webdriver.Chrome(options=options)

            # Obtener el titulo real del documento para nombrar el archivo.
            progress(8, "Obteniendo titulo")
            print("Obteniendo titulo del documento...")
            title = get_document_title(driver, input_url)
            if title:
                pdf_basename = sanitize_filename(title) + ".pdf"
                print(f"Titulo: {title}")
            else:
                pdf_basename = get_filename_from_url(input_url)
                print("No se obtuvo el titulo; uso el nombre de la URL.")
            pdf_filename = os.path.join(downloads_dir, pdf_basename)
            print(f"Output filename: {pdf_filename}")

            progress(15, "Cargando documento")
            ...
            # ── Via rapida para documentos de IMAGEN / escaneados ──
            # Si cada pagina es una imagen, bajamos los originales y los unimos
            # sin recomprimir: misma calidad (la de origen), mucho mas ligero y
            # sin margenes de impresion que recortar.
            image_urls = extract_page_image_urls(driver)
            if is_image_document(image_urls):
                print(f"\nDocumento de imagenes: {len(image_urls)} paginas.")
                print("Descargando imagenes originales (sin recomprimir)...")
                saved_path = download_images_to_pdf(image_urls, pdf_filename, converted_url)
                progress(100, "Listo")
                print(f"PDF saved successfully to: {saved_path}")
                return

            # ── Documento de TEXTO: render e impresion a PDF (texto vectorial) ──
            progress(45, "Preparando pagina")
